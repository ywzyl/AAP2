...
class Job:
    ...
    def create_job(self,request,env_desc,database_desc,failcount,subject):
        ...
        @sched.scheduled_job('cron', minute = self.task_schedule[0],hour = self.task_schedule[1],
                             day = self.task_schedule[2],month = self.task_schedule[3],
                             week = self.task_schedule[4],id=self.taskname)
        def task():
            #启动数据库和启动IP和获取邮件配置数据
